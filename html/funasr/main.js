...
btnAll.onmousedown = allstart
btnAll.onmouseup = stop
...
document.getElementById('wssip')
	.value = now_ipaddress;
addresschange();

function addresschange() {

	var Uri = document.getElementById('wssip')
		.value;
	document.getElementById('info_wslink')
		.innerHTML = "点此处手工授权（IOS手机）";
...
upfile.onchange = function() {
	...
	for (let i = 0; i < len; i++) {
		...
		file_ext = this.files[i].name.split('.')
			.pop()
			.toLowerCase();
		var audioblob;
...
function play_file() {
	...
	audio_record.src = (window.URL || webkitURL)
...
function on_recoder_mode_change() {
	...
	if (item == "mic") {
		document.getElementById("mic_mode_div")
			.style.display = 'block';
		document.getElementById("rec_mode_div")
			.style.display = 'none';
...
function getJsonMessage(jsonMsg) {
	...
	console.log("message: " + rectxt + ",asrmodel:" + asrmodel + ",is_final:" + is_final);
	if (asrmodel == "2pass-offline" || asrmodel == "offline") {
		//offline_text_current=rectxt;
		var varArea = document.getElementById('varArea');
		varArea.value = varArea.value + "\n" + getTime() + ":" + rectxt;
	...
	console.log("asr类型: " + asrmodel + ",当前值: " + rectxt + ",完整文本: " + offline_text);
	if (isfilemode == true && is_final == false) {
		//结束时候才执行语音聊天
		console.log("聊天文本:" + offline_text);
		chat(offline_text);
		//结束操作
		console.log("call stop ws【getJsonMessage】!");
		play_file();
		wsconnecter.wsStop();
...
function getConnState(connState) {
	if (connState === 0) { //on open
		...
		alert("连接地址" + document.getElementById('wssip')
			.value + "失败,请检查asr地址和端口。或试试界面上手动授权，再连接。");
		btnStart.disabled = true;
		btnStop.disabled = true;
		btnConnect.disabled = false;
...
function recProcess(buffer, powerLevel, bufferDuration, bufferSampleRate, newBufferIdx, asyncEnd) {
	if (isRec === true) {
		...
		var data_16k = Recorder.SampleData(array_48k, bufferSampleRate, 16000)
			.data;
