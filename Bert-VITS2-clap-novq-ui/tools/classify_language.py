...
def classify_language(text: str, target_languages: list = None) -> str:
    ...
    type = tools.sentence.check_lang_zhfirst(text)
    lang="zh"
    if type==1:
        lang = "zh"
    elif type==2:
        ...
        lang = "ja"
    print(f"{text}的语言权重：{lang},{type}")
    # winlone修改：当日语汉字和中文汉字重叠时候，优先中文
    # i=0
    # langs = classifier(text,k=3)
    # lang = langs[0][0]
    # print(f"{text}的语言权重：{langs}")
    # if lang=="ja":
    #     for str in langs[0]:
    #         if str=="zh" and langs[1][i]>0.001:
    #             print(f"{text}的优先语言：{str}")
    #             lang = "zh"
    #             break
    #         i=i+1
